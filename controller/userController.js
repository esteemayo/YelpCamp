...
exports.register = catchErrors(async (req, res, next) => {
  ...
  User.register(newUser, req.body.password, async (err, user) => {
    if (err) {
      ...
      return res.status(StatusCodes.OK).render('register', {
    ...
    try {
      await sendMail({
        ...
        text: `
          ...
          'host'
        )}/users/register/verify-email?token=${user.emailToken}
      `,
        html: `
          <h1>Hello,</h1>
      ...
      req.flash(
        'error',
        'Oops! Something went wrong. Please contact us for assistance.'
      );
      res.status(StatusCodes.OK).redirect('/');
    }
  });
...
exports.verifyEmail = catchErrors(async (req, res, next) => {
  ...
  if (!user) {
    req.flash(
      'error',
      'Oops! Token is invalid. Please contact us for assistance.'
    );
    return res.redirect('/');
  }
...
exports.updateUserData = catchErrors(async (req, res, next) => {
  ...
  res.status(StatusCodes.OK).redirect('/users/account/me');
});
...
exports.userProfile = catchErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('No user found with that ID', StatusCodes.BAD_REQUEST)
    );
  }

  res.status(StatusCodes.OK).render('users/profile', {
